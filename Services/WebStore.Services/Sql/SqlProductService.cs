...
namespace WebStore.Services.Sql
{
    public class SqlProductService : IProductService
    {
        ...
        public SaveRersultDto CreateProduct(ProductDto product)
        {
            try
            {
                _context.Products.Add(product.ToProduct());
                _context.SaveChanges();
                return new SaveRersultDto(true);
            }
            catch (DbUpdateConcurrencyException ex)
            {

                return new SaveRersultDto(false, ex.Message);
            }
        ...
        public SaveRersultDto UpdateProduct(ProductDto productDto)
        {
            var product = _context.Products.FirstOrDefault(p => p.Id == productDto.Id);
            if(product==null)
            {
                return new SaveRersultDto(false, "Entity does not exist");
            }
            product.BrandId = productDto.BrandId;
            product.SectionId = productDto.SectionId;
        ...
        public SaveRersultDto DeleteProduct(int productId)
        {
